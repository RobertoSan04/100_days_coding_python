...
1. **Prompt user** by asking: `"What would you like? (espresso/latte/cappuccino):"`
   ...
   - The prompt should show every time an action has completed, e.g. once the drink is dispensed. The prompt should show again to serve the next customer.
...
3. **Print report.**
   - When the user enters `report` in the prompt, a report should be generated that shows the current resource values, e.g.:
